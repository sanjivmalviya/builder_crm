...
<?php
   ...
   $transaction_types = getAll('tbl_transaction_types_master');

   $table_name = 'tbl_transaction_types_master';
   $field_name = 'id';
   ...
   if(isset($_POST['submit'])){
      ...
      if(isExists('tbl_transaction_types_master','name',$name)){

   			$error = "Oops ! Name already exists, try a different one";

      }else{
   
         if(insert('tbl_transaction_types_master',$form_data)){              
   			$success = "Transaction type Addded Successfully";
         }else{
            $error = "Failed to Add transaction type";
   		}		   		
   ...
   if(isset($_GET['delete_id'])){
      ...
      if(delete($table_name,$field_name,$_GET['delete_id'])){
         ...
         header('location:transaction_type.php');
      }else{
                     ...
                     <table class="table table-striped table-condensed table-bordered">
                        ...
                        <tbody>
                           <?php if(isset($transaction_types)){ ?>
                              <?php foreach($transaction_types as $rs){ ?>

                                 <tr>
                                    <td><?php echo $rs['name']; ?></td>
                                    <td class="text-right">
                                       <a href="transaction_type.php?edit_id=<?php echo $rs['id']; ?>"><i class="fa fa-pencil"></i></a>
                                       <a href="transaction_type.php?delete_id=<?php echo $rs['id']; ?>" onclick=" return confirm('Are you sure ?'); "><i class="fa fa-trash"></i></a>
                                    </td>
                                 </tr>
